...
module Decode = {
  ...
  let notifications = json : array(notification) =>
    Json.Decode.(array(notification))(json);
};
...
let markAsRead = (token, notification) =>
  Github.readNotification(token, notification.id);