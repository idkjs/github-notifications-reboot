...
let decodeGatekeeperResponse = json : string =>
  Json.Decode.(field("token", string))(json);
...
let requestToken = code => {
  ...
  Bs_fetch.fetchWithInit("https://api.github.com/notifications", reqInit)
  |> Js.Promise.then_(Bs_fetch.Response.json)
  |> Js.Promise.then_(json =>
       json |> decodeGatekeeperResponse |> Js.Promise.resolve
     );
};
...
let user = token => {
  let url = apiUrl("user");
  Bs_fetch.fetchWithInit(url, makeRequest(token))
  |> Js.Promise.then_(Bs_fetch.Response.text)
  |> Js.Promise.then_(jsonText => Js.Promise.resolve(jsonText));
};
...
let readNotification = (token, id) =>
  Bs_fetch.fetchWithInit(
    apiUrl("notifications/threads/") ++ id,
    makePatchRequest(token),
  )
  |> Js.Promise.then_(response =>
       Js.Promise.resolve(Fetch.Response.status(response) == 205)
     );